...
public class ReflectionUtils {
    ...
    public static synchronized ClassMate getClassMate(Class clazz) {
...
//            throw new IllegalStateException("Unexpected reflection exception - " + ex.getClass().getName() + ": " + ex.getMessage());
    ...
    private static ClassMate analyse(Class clazz) {
        if (CLASS_MAP.containsKey(clazz)) {
            //已经存在
            return CLASS_MAP.get(clazz);
        }
        ...
        if (targetClass != null && targetClass != Object.class) {
            ClassMate parentClassMate = analyse(targetClass);
            if(parentClassMate.getAutoIncrementInfo() != null) {
                classMate.setAutoIncrementInfo(parentClassMate.getAutoIncrementInfo());
            }
        }
    ...
    private static FieldMate getFieldMate(ClassMate classMate, Field field) {
        ...
        if (field.isAnnotationPresent(com.fangdd.traffic.common.mongo.annotation.Field.class)) {
            setFieldInfo(fieldMate, field.getAnnotation(com.fangdd.traffic.common.mongo.annotation.Field.class));
        }
        ...
        if (!ClassUtils.isSimpleType(fieldType)) {
            ...
            fieldTypeMate.getReferClassList().forEach(classMate::addReferClass);
            fieldMate.setSimpleType(false);
        }
        return fieldMate;
    }

    private static void setFieldInfo(FieldMate fieldMate, com.fangdd.traffic.common.mongo.annotation.Field field) {
        String fieldName = field.value();
        if (Strings.isNullOrEmpty(fieldName)) {
            return;
        }
        fieldMate.setAnnotationFiledName(fieldName);
    }
